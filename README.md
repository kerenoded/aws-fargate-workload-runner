...
This project intentionally uses **ECS RunTask on Fargate** as the execution model: **one task per run**, fully ephemeral.

### Why RunTask + Fargate (vs Lambda / ECS Service / EC2)
- **Ephemeral by default**: each run is isolated, leaves no long-running control plane, and terminates cleanly.
- **Predictable resources**: CPU/memory/network resources are consistent and isolated (unlike laptop-based tests affected by local throttling, background processes, VPNs, or Wi-Fi variance).
- **Better fit for load generators**: long-running, CPU/network-heavy workloads map more naturally to a long-lived process than to short-lived Lambda invocations.
- **No host management**: no EC2 capacity planning, patching, or cluster/node lifecycle.

### Why not Lambda for this
Lambda can work for small, bursty generators, but it’s a poor default for repeatable load generation:
- **Runtime limit**: max 15 minutes per invocation.
- **Concurrency quotas**: account/function concurrency limits can throttle the generator itself and can interfere with production capacity if you share an account.
- **Resource model**: CPU scales with memory, but you have less control over sustained networking/concurrency patterns compared to a long-lived process.
- **Cold starts & scaling behavior**: can add noise that’s unrelated to the target system you’re testing.

### Scaling model
- **One run = one task**: a single ECS task executes exactly one scenario with its own config + artifacts.
- **In-task concurrency** is controlled by the scenario (token bucket + worker pool).
- **Horizontal scale** is achieved by launching multiple tasks in parallel (multiple `RUN_ID`s), keeping artifacts isolated per run.

### Failure model & artifacts
- Runs are designed to be useful even when they fail:
  - logs go to CloudWatch Logs
  - metrics and summary are written locally and uploaded to S3 at the end
  - optional periodic uploads can preserve partial data on interruptions
- Exit codes are typed so automation can distinguish **config/infra errors** from **scenario/runtime errors**.

### Networking choice (default)
Tasks run in **public subnets** with a public IP for simplicity and cost:
- **no inbound access** (no inbound SG rules)
- **restricted egress** (typically TCP 443 for HTTPS and UDP/TCP 53 for DNS)

If your environment requires private-only egress, run tasks in private subnets and use NAT / egress proxy / VPC endpoints — the rest of the model stays the same.

### Control plane philosophy

The project intentionally keeps the control plane minimal.

A lightweight CLI launches tasks and stores configuration in S3.  
There is no persistent scheduler or orchestration layer.

Each run is independent and reproducible based solely on the
configuration and artifacts stored in S3.
...
| `artifacts_retention_days` | `30` | S3 lifecycle expiry (days) on artifacts under `runs/*` and `configs/*` objects |
...
**Networking:** ECS tasks run in **public subnets** with a public IP assigned. There are no inbound security group rules. Outbound traffic is restricted to HTTPS (443) for AWS API calls and DNS (53). No NAT Gateway is required or provisioned because tasks use public IPs for egress, which avoids the ~$32/month NAT Gateway cost.
